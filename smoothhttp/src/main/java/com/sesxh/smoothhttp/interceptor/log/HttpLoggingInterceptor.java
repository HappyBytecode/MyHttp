...
public final class HttpLoggingInterceptor implements Interceptor {
    ...
    public static final HttpLoggingInterceptor getInstance() {
     ...
     * you must call SmoothHttp.globalConfig().debug().
     *
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (level == Level.NONE ||
                !SmoothHttp.getInstance().globalConfig().isDebug()) {
        ...
        if (logHeaders) {
            ...
            for (String name : httpUrl.queryParameterNames()) {
                entity.addLog(name + ":" + httpUrl.queryParameter(name));
            }

            if (!logBody || !hasRequestBody || isIoRequest) {
                entity.addLog("Method: " + request.method());
            } else if (bodyHasUnknownEncoding(request.headers())) {
                entity.addLog("Method: " + request.method() + " (encoded body omitted)");
            } else {
